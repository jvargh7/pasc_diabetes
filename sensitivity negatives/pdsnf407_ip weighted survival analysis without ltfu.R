...
overlap_cox_fit <- coxph(as.formula(paste0("Surv(t, incident_tf) ~ COHORT + ",paste0(imbalanced_variables,collapse="+"))), 
                     data = cpitf_df, method='efron',weights = overlap_weight,cluster = ID)
...
saveRDS(ipw_cox_fit,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf407_ipw cox fit.RDS"))
saveRDS(overlap_cox_fit,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf407_overlap cox fit.RDS"))
saveRDS(ipw_cox_sex,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf407_ipw cox sex.RDS"))
saveRDS(ipw_cox_raceeth,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf407_ipw cox raceeth.RDS"))
saveRDS(ipw_cox_age,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf407_ipw cox age.RDS"))
saveRDS(ipw_cox_hospitalization,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf407_ipw cox hospitalization.RDS"))
...
bind_rows(
  ...
  write_csv(.,"sensitivity negatives/pdsnf407_coefficients from ip weighted survival analysis.csv")
