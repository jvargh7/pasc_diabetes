...
ipw_cox_fit <- readRDS(paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf401_ipw cox fit.RDS"))
overlap_cox_fit <- readRDS(paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf401_overlap cox fit.RDS"))
ipw_cox_sex <- readRDS(paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf401_ipw cox sex.RDS"))
ipw_cox_raceeth <- readRDS(paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf401_ipw cox raceeth.RDS"))
ipw_cox_age <- readRDS(paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf401_ipw cox age.RDS"))
ipw_cox_hospitalization <- readRDS(paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsnf401_ipw cox hospitalization.RDS"))
...
contrast_overlap <- map_dfr(1:nrow(difference_grid_overall),
                            function(i){
                              ...
                              # z_value = difference_grid_overall$modifier2_value[i]
                              bind_rows(
                                pdadm402_contrast_fit(overlap_cox_fit,x=x_name,y="") %>% 
                                  mutate(exposure = x_name,
                                         exposure_value = 1,
                                         modifier_value = NA_real_,
                                         outcome = "CPITF")) %>% 
...
bind_rows(contrast_overall,
  ...
  write_csv(.,"sensitivity negatives/pdsnf402_difference relative to exposed.csv")
