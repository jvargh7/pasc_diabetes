...
all_cpiom_df = bind_rows(readRDS(paste0(path_pasc_diabetes_folder,"/working/cleaned/pdsn001_cp otitis media.RDS")) %>% 
                         dplyr::select(ID,criterion1_date) %>% 
                           mutate(last_followup_date = criterion1_date) %>% 
                         mutate(incident_om = 1),
                       readRDS(paste0(path_pasc_diabetes_folder,"/working/cleaned/pdsn_noncpiom last followup.RDS")) %>% 
                         dplyr::select(ID,last_followup_date) %>% 
                         mutate(incident_om = 0))  %>% 
...
outcome_availability = demographic %>% 
  ...
  mutate(in_bmi_lookback_ID = case_when(ID %in% lookback_df$ID ~ 1,
                                        TRUE ~ 0),
         in_om_followup_ID = case_when(ID %in% all_cpiom_df$ID ~ 1,
                                        TRUE ~ 0))
