...
ipw_cox_age <-  coxph(as.formula(paste0("Surv(t, incident_om) ~ COHORT*age_category + ",paste0(imbalanced_variables,collapse="+"))), 
                      data = cpiom_df, method='efron',weights = sipw_age,cluster = ID)
summary(ipw_cox_age)
...
ipw_cox_hospitalization <-  coxph(as.formula(paste0("Surv(t, incident_om) ~ COHORT*hospitalization + ",paste0(imbalanced_variables,collapse="+"))), 
                                  data = cpiom_df, method='efron',weights = sipw_hospitalization,cluster = ID)
summary(ipw_cox_hospitalization)

saveRDS(ipw_cox_fit,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsno407_ipw cox fit.RDS"))
saveRDS(overlap_cox_fit,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsno407_overlap cox fit.RDS"))
saveRDS(ipw_cox_sex,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsno407_ipw cox sex.RDS"))
saveRDS(ipw_cox_raceeth,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsno407_ipw cox raceeth.RDS"))
saveRDS(ipw_cox_age,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsno407_ipw cox age.RDS"))
saveRDS(ipw_cox_hospitalization,paste0(path_pasc_diabetes_folder,"/working/sensitivity negatives/pdsno407_ipw cox hospitalization.RDS"))
...
bind_rows(
  ...
  write_csv(.,"sensitivity negatives/pdsno407_coefficients from ip weighted survival analysis.csv")
