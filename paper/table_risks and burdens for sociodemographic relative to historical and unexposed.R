...
pdadm403_relative_historical <-   read_csv("analysis cpit2dm/pdadm403_difference cumulative incidence from historical at one year.csv") %>% 
  dplyr::filter(time == 365) %>% 
  mutate(
    facet = modifier) %>% 
  mutate(facet = case_when(is.na(facet) ~ "Overall",
...
pdadm403_relative_unexposed <-   read_csv("analysis cpit2dm/pdadm403_difference cumulative incidence from unexposed at one year.csv") %>% 
  dplyr::filter(time == 365) %>% 
  mutate(
    facet = modifier) %>% 
  mutate(facet = case_when(is.na(facet) ~ "Overall",
...
bind_rows(pdadm405_hr_historical %>% 
  ...
  write_csv(.,"paper/table_risks and burdens for sociodemographic subgroups relative to historical and unexposed.csv")
