...
# Hazard ratios ----------
pdsu402_hr <- read_csv("sensitivity covid/pdsc402_difference relative to exposed.csv") %>% 
...
pdsu403_burden <-   read_csv("sensitivity covid/pdsc403_cumulative incidence at one year.csv") %>% 
...
pdsu403_relative <-   read_csv("sensitivity covid/pdsc403_difference cumulative incidence from exposed at one year.csv") %>% 
  dplyr::filter(time == 365) %>% 
  mutate(
    facet = modifier) %>% 
  mutate(facet = case_when(is.na(facet) ~ "Overall",
...
bind_rows(pdsu402_hr %>% 
  ...
  write_csv(.,"paper/table_sensitivity covid modeling results.csv")
