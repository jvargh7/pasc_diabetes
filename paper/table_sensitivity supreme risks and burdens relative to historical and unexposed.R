...
# Hazard ratios ----------
pdss405_hr_historical <- read_csv("sensitivity supreme/pdss405_difference relative to historical.csv") %>%
...
# Hazard ratios ----------
pdss406_hr_unexposed <- read_csv("sensitivity supreme/pdss406_difference relative to unexposed.csv") %>% 
  dplyr::filter(modifier_var != 'Overlap' | is.na(modifier_var)) %>% 
  mutate(group = str_replace(exposure,"COHORT",""),
         facet = str_replace(modifier1,modifier_var,"")) %>% 
  mutate(facet = case_when(is.na(modifier1) & modifier_var == "hospitalization_category" ~ "Not Hospitalized",
...
pdss403_burden <-   read_csv("sensitivity supreme/pdss403_cumulative incidence at one year.csv") %>% 
...
pdss403_relative_historical <-   read_csv("sensitivity supreme/pdss403_difference cumulative incidence from historical at one year.csv") %>% 
  dplyr::filter(time == 365) %>% 
  mutate(
    facet = modifier) %>% 
  mutate(facet = case_when(is.na(facet) ~ "Overall",
...
pdss403_relative_unexposed <-   read_csv("sensitivity supreme/pdss403_difference cumulative incidence from unexposed at one year.csv") %>% 
  dplyr::filter(time == 365) %>% 
  mutate(
    facet = modifier) %>% 
  mutate(facet = case_when(is.na(facet) ~ "Overall",
...
bind_rows(pdss405_hr_historical %>% 
            mutate(coef_ci = paste0(round(predicted,2)," (",
                                    round(conf.low,2),", ",
                                    round(conf.high,2),")")) %>% 
            mutate(type = "HR"),
          pdss406_hr_unexposed %>% 
            mutate(coef_ci = paste0(round(predicted,2)," (",
                                    round(conf.low,2),", ",
                                    round(conf.high,2),")")) %>% 
  ...
  write_csv(.,"paper/table_sensitivity supreme risks and burdens relative to historical and unexposed.csv")
