...
supremedm_followup_ID = analytic_sample %>%
...
(unweighted <- lookback_df %>% 
    ...
    left_join(all_supremedm_df %>% 
                dplyr::select(ID, CP, t, incident_dm),
              by = "ID") %>% 
    
    mutate(supremedm_availability = case_when(ID %in% supremedm_followup_ID ~ 1,
                                            TRUE ~ 2)
           
    ) %>% 
    mutate(
      supremedm_availability = factor(supremedm_availability,levels=c(1:2),
                                    labels=c("SUPREMEDM Available","SUPREMEDM Unavailable")),
      CP = case_when(ID %in% supremedm_followup_ID & incident_dm == 0 ~ "Censored",
                     TRUE ~ CP)
    ) %>% 
    tbl_summary(by = COHORT,
                include=c(female,age,
                          ...
                          landmark_cpit2dm, lookback_cpit2dm,supremedm_availability
                ),
                missing = "ifany",
                ...
                type = list(
                  ...
                  supremedm_availability ~ "categorical"
                ),
    ...
    add_overall()) %>%
  as_gt() %>%
  gt::gtsave(filename = "paper/table_supremedm computable phenotype summary.docx")
