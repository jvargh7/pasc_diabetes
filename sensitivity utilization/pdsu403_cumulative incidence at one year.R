...
(diff_with_historical = read_csv("sensitivity utilization/pdsu403_cumulative incidence at one year.csv") %>% 
    left_join(.,
              {.} %>% 
  ...
  write_csv(.,"sensitivity utilization/pdsu403_difference cumulative incidence from historical at one year.csv")

(diff_with_unexposed = read_csv("sensitivity utilization/pdsu403_cumulative incidence at one year.csv") %>% 
    left_join(.,
              {.} %>% 
  ...
  write_csv(.,"sensitivity utilization/pdsu403_difference cumulative incidence from unexposed at one year.csv")
