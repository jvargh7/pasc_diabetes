...
pdsu402_contrast_fit <- function(fit,x,y){
...
contrast_overall <- map_dfr(1:nrow(difference_grid_overall),
                            function(i){
                              ...
                              bind_rows(
                                pdsu402_contrast_fit(ipw_cox_fit,x=x_name,y="") %>% 
...
contrast_overlap <- map_dfr(1:nrow(difference_grid_overall),
                            function(i){
                              ...
                              # z_value = difference_grid_overall$modifier2_value[i]
                              bind_rows(
                                pdsu402_contrast_fit(overlap_cox_fit,x=x_name,y="") %>% 
                                  mutate(exposure = x_name,
                                         exposure_value = 1,
                                         modifier_value = NA_real_,
                                         outcome = "CPIT2DM")) %>% 
...
contrast_sex <- map_dfr(1:nrow(difference_grid_sex),
                        function(i){
                          ...
                          bind_rows(
                            pdsu402_contrast_fit(ipw_cox_sex,x_name,y_name) %>% 
                              mutate(exposure = x_name,
                                     modifier1 = y_name,
                                     exposure_value = 1,
                                     modifier1_value = 1,
...
contrast_age <- map_dfr(1:nrow(difference_grid_age),
                        function(i){
                          ...
                          bind_rows(
                            pdsu402_contrast_fit(ipw_cox_age,x_name,y_name) %>% 
                              mutate(exposure = x_name,
                                     modifier1 = y_name,
                                     exposure_value = 1,
                                     modifier1_value = 1,
...
contrast_raceeth <- map_dfr(1:nrow(difference_grid_raceeth),
                            function(i){
                              ...
                              bind_rows(
                                pdsu402_contrast_fit(ipw_cox_raceeth,x_name,y_name) %>% 
                                  mutate(exposure = x_name,
                                         modifier1 = y_name,
                                         exposure_value = 1,
                                         modifier1_value = 1,
...
contrast_hospitalization <- map_dfr(1:nrow(difference_grid_hospitalization),
                                    function(i){
                                      ...
                                      bind_rows(
                                        pdsu402_contrast_fit(ipw_cox_hospitalization,x_name,y_name) %>% 
                                          mutate(exposure = x_name,
                                                 modifier1 = y_name,
                                                 exposure_value = 1,
                                                 modifier1_value = 1,
...
bind_rows(contrast_overall,
  ...
  write_csv(.,"sensitivity utilization/pdsu402_difference relative to exposed.csv")
