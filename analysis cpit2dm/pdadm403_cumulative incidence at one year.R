...
(diff_with_exposed = surv_probs %>% 
  ...
  write_csv(.,"analysis cpit2dm/pdadm403_difference cumulative incidence from exposed at one year.csv")

surv_probs %>%
  mutate(across(one_of(c("surv","ci_lower","ci_upper")),.fns=function(x) (1-x)*1000,.names="cuminc_{col}")) %>% 
  write_csv(.,"analysis cpit2dm/pdadm403_cumulative incidence at one year.csv")

(diff_with_historical = read_csv("analysis cpit2dm/pdadm403_cumulative incidence at one year.csv") %>% 
    left_join(.,
              {.} %>% 
  ...
  write_csv(.,"analysis cpit2dm/pdadm403_difference cumulative incidence from historical at one year.csv")

(diff_with_unexposed = read_csv("analysis cpit2dm/pdadm403_cumulative incidence at one year.csv") %>% 
    left_join(.,
              {.} %>% 
  ...
  write_csv(.,"analysis cpit2dm/pdadm403_difference cumulative incidence from unexposed at one year.csv")
