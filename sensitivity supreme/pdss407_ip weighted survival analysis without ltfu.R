...
source("sensitivity supreme/pdss302_analytic dataset with ip weights for supremedm.R")
...
cox_fit <- coxph(Surv(t,incident_dm) ~ COHORT,data=supremedm_df, method='efron')
summary(cox_fit)
...
ipw_cox_fit <- coxph(as.formula(paste0("Surv(t, incident_dm) ~ COHORT + ",paste0(imbalanced_variables,collapse="+"))), 
                     data = supremedm_df, method='efron',weights = sipw,cluster = ID)
summary(ipw_cox_fit)

overlap_cox_fit <- coxph(as.formula(paste0("Surv(t, incident_dm) ~ COHORT + ",paste0(imbalanced_variables,collapse="+"))), 
                         data = supremedm_df, method='efron',weights = overlap_weight,cluster = ID)
...
ipw_cox_sex <- coxph(as.formula(paste0("Surv(t, incident_dm) ~ COHORT*sex_category + ",paste0(imbalanced_variables,collapse="+"))), 
                     data = supremedm_df, method='efron',weights = sipw_sex,cluster = ID)
summary(ipw_cox_sex)
...
data = supremedm_df %>% dplyr::filter(raceeth_category %in% c("NH White","NH Black","Hispanic")), 
...
ipw_cox_age <-  coxph(as.formula(paste0("Surv(t, incident_dm) ~ COHORT*age_category + ",paste0(imbalanced_variables,collapse="+"))), 
                      data = supremedm_df, method='efron',weights = sipw_age,cluster = ID)
summary(ipw_cox_age)
...
ipw_cox_hospitalization <-  coxph(as.formula(paste0("Surv(t, incident_dm) ~ COHORT*hospitalization + ",paste0(imbalanced_variables,collapse="+"))), 
                                  data = supremedm_df, method='efron',weights = sipw_hospitalization,cluster = ID)
summary(ipw_cox_hospitalization)

saveRDS(ipw_cox_fit,paste0(path_pasc_diabetes_folder,"/working/sensitivity supreme/pdss407_ipw cox fit.RDS"))
saveRDS(overlap_cox_fit,paste0(path_pasc_diabetes_folder,"/working/sensitivity supreme/pdss407_overlap cox fit.RDS"))
saveRDS(ipw_cox_sex,paste0(path_pasc_diabetes_folder,"/working/sensitivity supreme/pdss407_ipw cox sex.RDS"))
saveRDS(ipw_cox_raceeth,paste0(path_pasc_diabetes_folder,"/working/sensitivity supreme/pdss407_ipw cox raceeth.RDS"))
saveRDS(ipw_cox_age,paste0(path_pasc_diabetes_folder,"/working/sensitivity supreme/pdss407_ipw cox age.RDS"))
saveRDS(ipw_cox_hospitalization,paste0(path_pasc_diabetes_folder,"/working/sensitivity supreme/pdss407_ipw cox hospitalization.RDS"))
...
bind_rows(
  ...
  write_csv(.,"sensitivity supreme/pdss407_coefficients from ip weighted survival analysis.csv")
